...
def train_robust_linear_probe(
    ...
    for epoch, cpu_order in enumerate(progress):
        classifier.train()
        ...
        if epoch_hook is not None:
            epoch_hook(epoch, [classifier])

    if calibrate and global_margin is None:
    ...
    if not calibrate:
        raw = evaluate_classification(test_logits, test_labels)
        return {
            "accuracy": raw["top1"],
            "top5": raw["top5"],
            "macro_f1": raw["macro_f1"],
            "_artifacts": {"test_logits": test_logits},
        }
