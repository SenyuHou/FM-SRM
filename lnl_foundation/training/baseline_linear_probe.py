...
from lnl_foundation.training.real_evaluation import evaluate_classification
...
def _finish(models, test_features, test_labels, device, diagnostics, reduction="mean", compute_ece=True):
    ...
    evaluated = evaluate_logits(logits, test_labels) if compute_ece else evaluate_classification(logits, test_labels)
    metrics = {
        "accuracy": evaluated["accuracy"] if compute_ece else evaluated["top1"],
        "macro_f1": evaluated["macro_f1"],
        **diagnostics,
    }
    if compute_ece:
        metrics["ece_raw"] = evaluated["ece"]
    else:
        metrics["top5"] = evaluated["top5"]
    return BaselineResult(metrics, {
        "test_logits": logits,
        "test_predictions": logits.argmax(1),
        **({"test_probabilities_raw": evaluated["probabilities"]} if compute_ece else {}),
    })


def _train_single(train_features, noisy_labels, test_features, test_labels, num_classes,
                  seed, device, cfg, selected_indices=None, gce_q=None, epoch_hook=None, compute_ece=True):
    set_seed(seed)
    ...
    return _finish([model], test_features, test_labels, device, {"n_train": int(len(selected))}, compute_ece=compute_ece)


def _train_coteaching(train_features, noisy_labels, test_features, test_labels, num_classes,
                      seed, device, cfg, forget_rate, epoch_hook=None, compute_ece=True):
    set_seed(seed)
...
def _train_dividemix(train_features, noisy_labels, test_features, test_labels, num_classes,
                     dataset, noise_type, seed, device, cfg, epoch_hook=None, compute_ece=True):
    set_seed(seed)
    models = [_head(train_features.shape[1], num_classes, device) for _ in range(2)]
    optimizers = [_optimizer(model, cfg) for model in models]
    features, labels = train_features.to(device), noisy_labels.to(device)
    ...
    for epoch in progress:
        ...
        if epoch_hook is not None:
            epoch_hook(epoch, models, reduction="sum")
    return _finish(models, test_features, test_labels, device, {
    ...
    }, reduction="sum", compute_ece=compute_ece)
...
def _train_disc(train_features, noisy_labels, test_features, test_labels, num_classes,
                noise_type, noise_rate, seed, device, cfg, epoch_hook=None, compute_ece=True):
    set_seed(seed)
    model = _head(train_features.shape[1], num_classes, device)
    optimizer = _optimizer(model, cfg)
...
def train_stage2_baseline(method, train_features, noisy_labels, test_features, test_labels,
                          ...
                          selected_indices=None, epoch_hook=None, compute_ece=True):
    ...
    if method in {"ce", "clipcleaner"}:
        return _train_single(
            ...
            epoch_hook=epoch_hook, compute_ece=compute_ece,
        )
    if method == "gce":
        return _train_single(
            train_features, noisy_labels, test_features, test_labels, num_classes, seed,
    ...
    if method == "dividemix":
        return _train_dividemix(
            ...
            noise_type, seed, device, method_cfg, epoch_hook=epoch_hook, compute_ece=compute_ece,
        )
    return _train_disc(
        train_features, noisy_labels, test_features, test_labels, num_classes, noise_type,
        noise_rate, seed, device, method_cfg, epoch_hook=epoch_hook, compute_ece=compute_ece,
    )
