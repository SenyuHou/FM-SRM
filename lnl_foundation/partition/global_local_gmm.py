...
class GlobalLocalGMMPartitioner:
    ...
    def _compute_local_consistency(self, features: torch.Tensor, noisy_labels: torch.Tensor) -> np.ndarray:
        ...
        if len(features) > 100_000:
            neighbor_idx = self._scalable_neighbors(features)
            labels = noisy_labels.numpy()
            return (labels[neighbor_idx] == labels[:, None]).mean(axis=1).astype(np.float32)
    ...
    def _scalable_neighbors(self, features: torch.Tensor) -> np.ndarray:
        """Exact cosine top-k, excluding self; memory is O(batch * N), never O(N^2)."""
        normalized = F.normalize(features.float(), dim=1).contiguous()
        ...
        if faiss is not None:
            ...
            index.add(values)
            result = np.empty((count, self.k), dtype=np.int64)
            for start in range(0, count, 1024):
                stop = min(start + 1024, count)
                _, neighbors = index.search(values[start:stop], self.k + 1)
                for offset, row in enumerate(neighbors):
                    others = row[row != start + offset]
                    result[start + offset] = others[:self.k]
            return result
        result = np.empty((count, self.k), dtype=np.int64)
        # Cap similarity chunk to approximately 64 MiB on CPU; CUDA may use
        # larger chunks while still avoiding a full pairwise matrix.
        batch_size = max(1, min(256, 16_000_000 // count))
        for start in range(0, count, batch_size):
            stop = min(start + batch_size, count)
            similarities = normalized[start:stop] @ normalized.T
            similarities[torch.arange(stop - start), torch.arange(start, stop)] = -float("inf")
            result[start:stop] = similarities.topk(self.k, dim=1).indices.numpy()
        return result
